...
int createSocket(int socket_domain)
{
...
//Binding
void socketBinding(int fd, struct sockaddr *server, int portNumber)
{   
...
char *getHostName(struct sockaddr *client, char *buffer)
{
    switch(client->sa_family){
        case AF_INET:{
            hostEntry = gethostbyaddr(&(client->sa_addr), sizeof(struct sa_addr), AF_INET);
            if (hostEntry == NULL)
            {
                buffer = "Unkown Host";
            }
        ...
        case AF_UNIX:{
            hostEntry = gethostbyaddr(&(client->sa_addr), sizeof(struct sa_addr), AF_UNIX);
            if (hostEntry == NULL)
            {
                buffer = "Unkown Host";
            }
...
int acceptConnection(int fd, struct sockaddr *client)
{
    switch(client->sa_family){
        case AF_INET:{
...
/*void deleteFile(int client, int fd, char *buffer, char *pathname){
    struct sockaddr clientAddr;
    socklen_t length;
    if(getsockname(fd, &clientAddr, (socklen_t *)&length))
}*/

int main()
{
    int dataSocketfd, datafd;
    int adminfd = createSocket(AF_UNIX);                       //create a local socket
    int listenfd = createSocket(AF_INET);                       //create inet socket to listen for connections
    ...
    while (true)
    {
        int connectafd = acceptConnection(adminfd, &clientAddr); 
        ...
        else
        {
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    if (strcmp("D\n", buffer) == 0)
                    {
                        dataSocketfd = createSocket(AF_INET);
